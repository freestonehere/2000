...
void endPaint()
{
	DeleteDC(g_hmemdc);	
	g_hmemdc = 0;
    InvalidateRect(g_hWnd,0,0); //触发WM_PAINT消息
...
void loadImage(HINSTANCE hInstance, int resourceId, ACL_Image* pImage) {
	// 加载位图资源
	HBITMAP hBitmap = LoadBitmap(hInstance, MAKEINTRESOURCE(resourceId));

	if (hBitmap) {
		// 如果加载成功，设置 ACL_Image 结构
		...
		// 设置位图尺寸
		pImage->width = bmp.bmWidth;
		pImage->height = bmp.bmHeight;
	}
	else {
		// 加载失败，可以设置错误代码或进行错误处理
		pImage->hBitmap = NULL;
	}
}
